...
public class WeaponManager {
    ...
    // Tracks which players HAD a cooldown last tick (for expiry detection)
    private final Set<UUID> hadCooldownLastCheck = new HashSet<>();
    ...
    public boolean isOnCooldown(Player player) {
        ...
        if (expiry == null) {
            hadCooldownLastCheck.remove(id);
            return false;
        }
        ...
        hadCooldownLastCheck.add(id);
        return true;
    }

    /** Returns true agar player ka cooldown PICHLE check mein tha (expiry detect karne ke liye) */
    public boolean hadCooldown(Player player) {
        return hadCooldownLastCheck.contains(player.getUniqueId());
    }
