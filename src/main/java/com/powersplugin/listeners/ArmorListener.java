...
public class ArmorListener implements Listener {
    ...
    // Prevents the ability firing twice if both PlayerInteractEvent and
    // PlayerInteractEntityEvent fire from the same physical click.
    private final Map<UUID, Long> lastTrigger = new HashMap<>();
    private static final long TRIGGER_DEBOUNCE_MS = 300;
    ...
    // ─── Sonic Boom trigger #1: Shift + Right Click on air/block ──────
    @EventHandler(priority = EventPriority.HIGH)
    public void onPlayerInteract(PlayerInteractEvent e) {
        Action action = e.getAction();
        if (action != Action.RIGHT_CLICK_AIR && action != Action.RIGHT_CLICK_BLOCK) return;
        tryActivate(e.getPlayer());
    }

    // ─── Sonic Boom trigger #2: Shift + Right Click on an entity/player ──
    @EventHandler(priority = EventPriority.HIGH)
    public void onPlayerInteractEntity(PlayerInteractEntityEvent e) {
        // Only the main-hand interaction should trigger this, not off-hand echo
        if (e.getHand() != org.bukkit.inventory.EquipmentSlot.HAND) return;
        if (tryActivate(e.getPlayer())) {
            e.setCancelled(true); // prevent accidentally interacting with the entity looked at
        }
    }

    // ─── Shared activation logic with debounce ────────────────────────
    private boolean tryActivate(Player player) {
        if (!player.isSneaking()) return false;
        if (!isWearingAnyGodArmor(player)) return false;

        UUID id = player.getUniqueId();

        // Debounce: ignore if we just fired this within the last 300ms
        Long last = lastTrigger.get(id);
        long now = System.currentTimeMillis();
        if (last != null && (now - last) < TRIGGER_DEBOUNCE_MS) return true; // already handled
        lastTrigger.put(id, now);

        if (sonicBoomCooldown.containsKey(id)) {
            long remaining = sonicBoomCooldown.get(id) - now;
            if (remaining > 0) {
        ...
        sonicBoomCooldown.put(id, now + SONIC_BOOM_COOLDOWN_MS);
