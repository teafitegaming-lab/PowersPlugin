    ...
    public void onPlayerDeath(PlayerDeathEvent e) {
        ...
        if (plugin.getPowerManager().hasPower(dead, CustomPower.CELESTIAL_KNIGHT)) {
            ...
            new org.bukkit.scheduler.BukkitRunnable() {
                @Override public void run() {
                    ...
                    deathLoc.getWorld().spawnParticle(Particle.END_ROD,
                            deathLoc.add(0, 1, 0), 50, 0.5, 1, 0.5, 0.5);
                    deathLoc.getWorld().playSound(deathLoc, Sound.ITEM_TOTEM_USE, 1f, 1f);
        ...
        if (plugin.getPowerManager().hasPower(dead, CustomPower.VOID_WALKER)) {
            ...
            new org.bukkit.scheduler.BukkitRunnable() {
                @Override public void run() {
                    dead.setHealth(2.0);
                    Location safe = deathLoc.clone().add(
