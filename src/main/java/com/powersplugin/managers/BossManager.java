...
public class BossManager {
    ...
    // ─── Summon Items (crafted separately, combined to spawn boss) ───────
    ...
    public void spawnBoss(Location location) {
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                bossBar.setProgress(Math.max(0, Math.min(1, le.getHealth() / maxHp)));
                bossBar.setTitle("§4⚔ ANCIENT GUARDIAN §7[" + (int) le.getHealth() + "/" + (int) maxHp + "] ⚔");

                // Particle aura
                location.getWorld().spawnParticle(Particle.SOUL_FIRE_FLAME, le.getLocation().add(0,1,0), 10, 1, 1, 1, 0.1);
    ...
    public void onBossDeath(Location loc) {
        ...
        // Drop the One-Shot Sword — only obtainable this way
        loc.getWorld().dropItemNaturally(loc, plugin.getWeaponManager().createWeapon(CustomWeapon.CELESTIAL_JUDGMENT));
        ...
        for (int i = 0; i < 5; i++) {
            ...
            new BukkitRunnable() {
                @Override public void run() {
                    loc.getWorld().spawnParticle(Particle.EXPLOSION_EMITTER, loc.clone().add(
                            (Math.random()-0.5)*4, Math.random()*3, (Math.random()-0.5)*4), 5, 0,0,0,0);
                    loc.getWorld().spawnParticle(Particle.END_ROD, loc.clone().add(0,1,0), 30, 2,2,2,0.3);
        ...
        String msg = plugin.getConfig().getString("boss.death-message",
                "&a&lTHE ANCIENT GUARDIAN HAS BEEN DEFEATED! THE CELESTIAL JUDGMENT SWORD DROPS!");
        plugin.getServer().broadcast(LegacyComponentSerializer.legacyAmpersand().deserialize(msg));
    }
