...
public class ArmorListener implements Listener {
    ...
    // ─── God Armor damage reduction (full set) ──────────────────────
    ...
    private void activateSonicBoom(Player player) {
        ...
        world.spawnParticle(Particle.EXPLOSION_LARGE, origin, 5, 1, 0.5, 1, 0);

        // ── Player screen effect ──────────────────────────────────────
        player.addPotionEffect(new PotionEffect(PotionEffectType.CONFUSION, 40, 0, false, false));
        ...
        for (Player target : targets) {
            ...
            target.addPotionEffect(new PotionEffect(PotionEffectType.SLOW, 120, 3));
            target.addPotionEffect(new PotionEffect(PotionEffectType.WEAKNESS, 200, 2));

            // Shatter message + title
            ...
            // Staggered armor break animation
    ...
    private void breakArmorWithAnimation(Player target) {
        ...
        // Instant visual burst on target
        target.getWorld().spawnParticle(Particle.EXPLOSION_LARGE, loc, 5, 0.5, 0.5, 0.5, 0);
    ...
    private void crackSound(Player target) {
        ...
        target.getWorld().spawnParticle(Particle.EXPLOSION_LARGE, loc, 2, 0.2, 0.2, 0.2, 0);
    }

    // ─── Helper: any god armor piece equipped ─────────────────────────
